...
class BasicRNNCell_MulInt(RNNCell):
    ...
    def __call__(self, inputs, state, timestep=0, scope=None):
        ...
        with tf.variable_scope(scope or type(self).__name__):  # 'BasicRNNCell'
            output = tf.tanh(multiplicative_integration(
...
class GRUCell_MulInt(RNNCell):
    ...
    def __init__(self, num_units, gpu_for_layer=0, weight_initializer='uniform_unit',
                 orthogonal_scale_factor=1.1, use_highway=False, num_highway_layers=2,
        ...
        with tf.variable_scope(scope or type(self).__name__):  # 'GRUCell'
                ...
                r, u = tf.split(
                    value=tf.sigmoid(
                        multiplicative_integration(
                            [inputs, state],
                            output_size=self._num_units * 2, initial_bias_value=1.0)),
            ...
            with tf.variable_scope('Candidate'):
                ...
                c = tf.tanh(multiplicative_integration(
                    [inputs, state], self._num_units, 0.0))
...
class BasicLSTMCell_MulInt(RNNCell):
    ...
    def __init__(self, num_units, forget_bias=1.0, gpu_for_layer=0,
    ...
    def __call__(self, inputs, state, timestep=0, scope=None):
        ...
        with tf.variable_scope(scope or type(self).__name__):  # 'BasicLSTMCell'
            ...
            concat = multiplicative_integration(
                [inputs, h], output_size=self._num_units * 4, initial_bias_value=0.0)
            ...
            if self.use_recurrent_dropout and self.is_training:
                input_contribution = tf.nn.dropout(
                    tf.tanh(j), keep_prob=self.recurrent_dropout_factor)
            else:
