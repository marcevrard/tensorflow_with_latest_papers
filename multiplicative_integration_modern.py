...
'''the classes here contain integrative multiplication versions of the RNN
which converge faster and lead to better scores
    ...
    with tf.variable_scope(scope or 'double_inputs_multiple_integration'):
        ...
        if weights_already_calculated:  # if already have weights to insert from batch norm
            ...
            with tf.variable_scope('Calculate_Wx_mulint'):
                Wx = linear.linear(list_of_inputs[0], output_size, bias=False,
                                   use_l2_loss=use_l2_loss, timestep=timestep)
            with tf.variable_scope('Calculate_Uz_mulint'):
                Uz = linear.linear(list_of_inputs[1], output_size, bias=False,
                                   use_l2_loss=use_l2_loss, timestep=timestep)
